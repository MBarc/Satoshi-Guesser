...
// running on GitHub Actions. The estimate uses three throughput phases that
...
//   Phase 3 (post-Plan 8):    ~1.25 trillion keys/day  — native Rust addon
//
...
const CURRENT_RATE_PER_SECOND = RATES_PER_DAY.phase3 / SECONDS_PER_DAY;
...
function estimatedKeysSinceLaunch(now = Date.now()) {
  ...
  const p3 = seconds(PHASE_BOUNDARIES.PLAN_8, now)     * (RATES_PER_DAY.phase3 / SECONDS_PER_DAY);

  return Math.floor(p1 + p2 + p3);
}
